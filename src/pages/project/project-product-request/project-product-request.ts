...
@Component({
  selector: 'project-product-request',
  templateUrl: 'project-product-request.html',
})
export class ProjectProductRequestsPage {
  busy: any;
  ...
  initListDisplayData(){
      ...
      console.log('this.navParams.data',this.navParams.data);
         ...
         return new Promise((resolve,reject)=>{
            let projId = (this.navParams.data['projId'])?this.navParams.data['projId']:0;  
            let queryProdReq="SELECT DISTINCT prt.id,prt.server_id,prt.request_date,prt.product_request_status,prt.product_request_status_remark,prt.quantity_required,prt.new_price_request,prt.term_of_payment,prt.pic_same_as_hpb,prt.pic_name,prt.pic_designation,prt.pic_mobile,prt.additional_comments,prt.hpb_digital_sign,prm.project_id,prm.server_project_id,prm.project_name,hm.hpb_id,hm.server_hpb_id,hm.hpb_name, rdm.rds_id,rdm.server_rds_id,rdm.rds_name FROM products_request_tbl prt LEFT JOIN project_master prm ON prt.project_id=prm.project_id LEFT JOIN hpb_master hm ON prt.server_hpb_id=hm.server_hpb_id LEFT JOIN retailer_distributor_master rdm ON prt.rds_id=rdm.server_rds_id WHERE prm.project_id="+projId+" ORDER BY prt.local_created_date DESC";
