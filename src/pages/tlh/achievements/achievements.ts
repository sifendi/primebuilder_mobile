...
export class TlhAchievementsPage {
    sphData:any = [];
    sphSelect:any = 'all';
    sphStatsData:any;
    srkuData:any = [];
    ...
    console.log("todayActualPerc",this.todayActualPerc);
        ...
        let allQuery = "select sum(monthly_target) as monthly_target,sum(achieved_target) as achieved_target,sum(target_remaining) as target_remaining,sum(todays_achievement) as todays_achievement,sum(todays_target) as todays_target,target_for from home_stats_ac_tlh where date(datetime(stats_date / 1000 , 'unixepoch','localtime')) = date('"+currDate+"') group by target_for";
        console.log(" allQuery --",allQuery);
        this.sqlS.selectTableQueryData(allQuery,[]).then((allresData:any)=>{
    ...
    getSphStats(){
        ...
        this.prodArrObj = [];
        let index = 0;
        if(this.sphSelect == 'all'){
            queryHomeStats = "select sum(monthly_target) as monthly_target,sum(achieved_target) as achieved_target,sum(target_remaining) as target_remaining,sum(todays_achievement) as todays_achievement,sum(todays_target) as todays_target,target_for from home_stats_ac_tlh where date(datetime(stats_date / 1000 , 'unixepoch','localtime')) = date('"+currDate+"') group by target_for";
        }else{
            queryHomeStats = "select * from home_stats_ac_tlh where sphid = '"+this.sphSelect+"' and date(datetime(stats_date / 1000 , 'unixepoch','localtime')) = date('"+currDate+"')";
        }
