...
import { TlhHpbProductRequestsDetailsPage } from "../tlh-hpb-pages/tlh-hpb-product-requests-details/tlh-hpb-product-requests-details";
import * as moment from 'moment';
import { TlhProductRequestsSearchPage } from "../tlh-product-requests-search/tlh-product-requests-search";

declare var sessionUserGlobalData;

@Component({
  selector: 'tlh-product-request-list',
  templateUrl: 'tlh-product-request-list.html',
})
export class TlhProductRequestsListPage {

  busy: any;
  busyMessage: any;
  productRequestAllData:any=[];
  productRequestAllDataTemp=[];
  limit:number = 5;
  pages:number = 0;
  dataLen:number = 0;
  uId:number;
  ...
  loadData(){
    return new Promise((resolve,reject)=>{
        ...
        this.busy =  this.productRequestApi.getProductRequest(null,null,null,null,null,this.uId,this.role,this.limit,this.pages,null,null,this.prodRequestFilterArr.fromDate,this.prodRequestFilterArr.toDate,this.prodRequestFilterArr.status,this.searchString).subscribe(resData => {
        this.pages++;  
...
openProductRequestFilter(){
        ...
        let filterPageD = this.modalCtrl.create(TlhProductRequestsSearchPage,{prodRequestFilterArr:this.prodRequestFilterArr});
