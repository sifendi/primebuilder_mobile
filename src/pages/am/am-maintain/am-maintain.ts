...
@Component({
  ...
  templateUrl: 'am-maintain.html',
})
export class amMaintainPage {
      ...
      this.sqlS.selectTableQueryData(queryHomeStatsDist,[]).then((ressqlDataN:any)=>{
            ...
            for(let j=0;j<ressqlDataN.rows.length;j++){
                ...
                if(j == ressqlDataN.rows.length-1){
                    ...
                    let queryHomeStats = "select * from home_stats_ac_tlh where target_for = 'cement_vol_maintain' and sphid IN ("+sphIds+")";
