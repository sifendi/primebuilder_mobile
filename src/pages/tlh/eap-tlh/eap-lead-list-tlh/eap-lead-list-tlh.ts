...
import { EapLeadSearchModalTlh } from "../eap-lead-list-search-modal-tlh/eap-lead-list-search-modal-tlh";
import { EapLeadDetailsTlh } from "../eap-lead-details-tlh/eap-lead-details-tlh";
declare var sessionUserGlobalData;
@Component({
  selector: 'eap-lead-list-tlh',
  templateUrl: 'eap-lead-list-tlh.html',
})
export class EapLeadListTlh {
 ...
 async initListData(){
  ...
  goToDetails(leadId,leadData){
      this.navCtrl.push(EapLeadDetailsTlh,{leadId:leadId,leadData:leadData});
  }

  openSearch(){
    console.log('ionViewDidLoad openSearch');
    let filterDataObj={};
    let modelSearch = this.modCtrl.create(EapLeadSearchModalTlh,filterDataObj);
