...
@Component({
  selector: 'visit-contractor-list-page',
  templateUrl: 'visit-contractor-list-page.html',
})
export class VisitContractorListPage {
    ...
    console.log('currDate',currDate);
     ...
     this.appCom.getAppPreference("userCreds").then((resDataU)=>{
            ...
            if( resDataU != undefined && resDataU != '' ){
                ...
                let querySelectTemp = "select * from todays_temp_target where date(datetime(local_created_date / 1000 , 'unixepoch','localtime')) = date('"+currDate+"') AND for IN('myPlan_contractor_new_member','myPlan_contractor_visit','myPlan_contractor_maintain','myPlan_contractor_switching','myPlan_contractor_srku')";
                ...
                this.busy=this.sqlS.selectTableQueryData(querySelectTemp,[]).then((resTempData:any)=>{
                    if(resTempData.rows.length > 0){
                        for(let i = 0;i < resTempData.rows.length;i++){
                            ...
                            if(resTempData.rows.item(i).for == 'myPlan_contractor_new_member'){
                                this.newMemberTotal = resTempData.rows.item(i).achieved;
                            }else if(resTempData.rows.item(i).for == 'myPlan_contractor_visit'){
                                this.visitTotal = resTempData.rows.item(i).achieved;
                            }else if(resTempData.rows.item(i).for == 'myPlan_contractor_maintain'){
                                this.maintainTotal = Math.round(resTempData.rows.item(i).achieved * 100) / 100;
                            }else if(resTempData.rows.item(i).for == 'myPlan_contractor_switching'){
                                this.switchingTotal = Math.round(resTempData.rows.item(i).achieved * 100) / 100;
                            }else if(resTempData.rows.item(i).for == 'myPlan_contractor_srku'){
                ...
                this.sqlS.selectTableQueryData(queryGetTodaysTotalCheckins,[]).then((ressqlGetData:any)=>{
                    ...
                    for(let i = 0;i<ressqlGetData.rows.length;i++){
                        if(ressqlGetData.rows.item(i).check_in_datetime != null){
                            ...
                            let tempObj = ressqlGetData.rows.item(i);
                            let tempObjPic = JSON.parse(ressqlGetData.rows.item(i).hpb_profile_pic);           tempObj.visitStatus = false;
                            tempObj.proPic = tempObjPic[0].path;
     ...
     this.appCom.getAppPreference("userCreds").then((resDataU)=>{
            ...
            if( resDataU != undefined && resDataU != '' ){
                this.uid=resDataU.userId;
                    ...
                    //  Todays total visits
                    let queryGetTodaysTotalCheckinsQ = "select pm.*,hm.* from hpb_master as hm JOIN project_master as pm ON hm.server_hpb_id = pm.server_hpb_id JOIN check_in_out as cio ON pm.project_id = cio.check_in_out_type_id where cio.check_in_out_user_id = '"+this.uid+"' and date(datetime(cio.local_created_date / 1000 , 'unixepoch','localtime')) = date('"+currDate+"') GROUP BY hm.server_hpb_id";
                    ...
                    let queryGetTodaysTotalConsumption = "select pm.*,hm.*,prm.*,(SELECT prat.approval_status FROM products_receipt_approval_tbl AS prat WHERE prat.receipt_id = prm.receipt_id AND prat.approval_role = 'TLH') AS tlh_approval,(SELECT prat.approval_status FROM products_receipt_approval_tbl AS prat WHERE prat.receipt_id = prm.receipt_id AND prat.approval_role = 'AC') AS ac_approval,(SELECT prat.approval_status FROM products_receipt_approval_tbl AS prat WHERE prat.receipt_id = prm.receipt_id AND prat.approval_role = 'SA') AS sa_approval from hpb_master as hm JOIN product_receipt_master as prm ON hm.server_hpb_id = prm.server_hpb_id JOIN product_master AS pm ON pm.server_product_id = prm.product_id where prm.created_by = '"+this.uid+"' and date(datetime(prm.local_created_date / 1000 , 'unixepoch','localtime')) = date('"+currDate+"') and hm.hpb_status IN('maintain','switching') and hm.hpb_type = '"+hpb_type+"'";
