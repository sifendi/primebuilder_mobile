...
@Component({
  selector: 'page-am-achievements-sph',
  templateUrl: 'am-achievements-sph.html',
})
export class AmAchievementsSphPage {
  sphData:any = [];
  sphSelect:any;
  sphStatsData:any;
  srkuData:any = [];
  ...
  @ViewChild('sphSelectId') sphSelectId: any;
  @ViewChild('sphSelectMob') sphSelectMob: any;
    ...
    this.appCom.getAppPreference("userCreds").then((resDataU)=>{
      ...
      this.sqlS.selectTableQueryData(queryHomeStats,[]).then((ressqlData:any)=>{
          ...
          for(let i=0;i<ressqlData.rows.length;i++){
              ...
              this.sphData.push({"text":temp.name,"value":temp.id});
              //this.sphData.push(temp);
          }
          this.sphSelectMob.instance.option({
            data: this.sphData
        });
         
          if( this.sphData != undefined && this.sphData != ''  ){
             if( this.sphData[0] ){
                  this.sphSelect=this.sphData[0]['value'];
                    setTimeout(()=>{
                        console.log("this.sphData[0]['value']",this.sphData[0]['value']);
                        this.sphSelectId.valueAccessor._instance.setVal(this.sphData[0]['value'],true);
                        this.getSphStats();     
                    },300);
             }
          }
  ...
  mobiScollReferCustSettings: any = {
    ...
    // onFilter: (event, inst)=> {
    //         this.filtersphMobi(event.filterText);
    // }
  };
  ...
  getSphStats(){
    ...
    filtersphMobi(serchKey?:any){
      this.sphData=[];
      let currDate = moment().format("YYYY-MM-DD").toString(); 
      let queryHomeStats = "select distinct('hsat.sphid') as sphid,hsat.*,ud.* from home_stats_ac_tlh as hsat JOIN user_data as ud on ud.uid = hsat.sphid where date(datetime(stats_date / 1000 , 'unixepoch','localtime')) = date('"+currDate+"') GROUP BY sphid";
       if(serchKey){
           queryHomeStats="select distinct('hsat.sphid') as sphid,hsat.*,ud.* from home_stats_ac_tlh as hsat JOIN user_data as ud on ud.uid = hsat.sphid where date(datetime(stats_date / 1000 , 'unixepoch','localtime')) = date('"+currDate+"') and name  LIKE '%"+serchKey+"%' GROUP BY sphid " ;
        }
      this.sqlS.selectTableQueryData(queryHomeStats,[]).then((ressqlData:any)=>{
