...
@Component({
  ...
  templateUrl: 'am-switching.html',
})
export class amSwitchingPage {
  ...
  constructor(private events:Events,private shareS:ShareService,private sqlS:SqlServices,public navCtrl: NavController, public navParams: NavParams) {
    ...
    ionViewDidEnter(){
        ...
        console.log(" master sync status ionViewDidEnter ac MaintainPage",masterSync);
        if(!masterSync){
            this.firstFunction();      
        }
    }
      ...
      this.sqlS.selectTableQueryData(queryHomeStatsDist,[]).then((ressqlDataN:any)=>{
            ...
            for(let j=0;j<ressqlDataN.rows.length;j++){
                ...
                if(j == ressqlDataN.rows.length-1){
                    ...
                    let queryHomeStats = "select * from home_stats_ac_tlh where target_for = 'cement_vol_switching' and sphid IN ("+sphIds+")";
