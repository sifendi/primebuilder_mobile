...
@Component({
  selector: 'page-notifications-new',
  templateUrl: 'notifications-new.html',
})
export class NotificationsNewPage {
  busy:any;
 ...
 initData() {
  ...
  notificationNewLoadIfTLHAC(){
      ...
      this.busy=this.appNotifyCenApi.getNotifications(null,this.currentUserSessionData['userId'],null,"0").subscribe((respResult:any)=>{
  ...
  notificationNewLoadIfSPH(){
     console.log('notificationNewLoadIfSPH');
      ...
      let query="SELECT DISTINCT nc.server_ntc_id, nc.*,prm.receipt_id, "+projectFileds+" FROM notification_center nc LEFT JOIN product_receipt_master prm on  nc.ntc_type_id=prm.server_receipt_id LEFT JOIN project_master pm on nc.ntc_type_id=pm.server_project_id WHERE nc.ntc_user_read_flag=0 AND nc.status=1 ORDER BY nc.created_date DESC";
