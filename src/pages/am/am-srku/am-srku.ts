...
@Component({
  selector: 'page-srku',
  templateUrl: 'am-srku.html',
})
export class amSrkuPage {
  ...
  constructor(private events:Events,private shareS:ShareService,private sqlS:SqlServices,public navCtrl: NavController, public navParams: NavParams) {
    ...
    ionViewDidEnter(){
        ...
        console.log(" master sync status ionViewDidEnter ac SrkuPage",masterSync);
        if(!masterSync){
            this.firstFunction();      
        }
    }
  ...
  firstFunction() {
      console.log('ionViewDidLoad ac SrkuPage');
      ...
      if(selectedDistrict == 'all' || selectedDistrict == false){
        queryHomeStatsDist = "SELECT DISTINCT(uid) FROM user_data, json_tree(user_data.district) where role = '$sph'";
      }else if(isNaN(selectedDistrict) == false){
        queryHomeStatsDist = "SELECT DISTINCT(uid) FROM user_data, json_tree(user_data.district) where json_tree.key = 'id' and json_tree.value IN ("+selectedDistrict+") and role = '$sph'";    
      }
      console.log(" queryHomeStatsDist ",queryHomeStatsDist);
      this.sqlS.selectTableQueryData(queryHomeStatsDist,dataArr).then((ressqlDataN:any)=>{
            ...
            for(let j=0;j<ressqlDataN.rows.length;j++){
                ...
                if(j == ressqlDataN.rows.length-1){
                    ...
                    let queryHomeStats = "select * from home_stats_ac_tlh where target_for = 'srku_vol' and sphid IN ("+sphIds+")";
                    ...
                    this.sqlS.selectTableQueryData(queryHomeStats,dataArr).then((ressqlData:any)=>{
                            ...
                            for(let i=0;i<ressqlData.rows.length;i++){
                                ...
                                //console.log(" stats ",tempSrkuTodayT,tempSrkuTodayA,tempSrkuMonthlyT,tempSrkuMonthlyA);
